...
parser.add_argument("foldername", help="name of folder that has files to be appended to output.csv")
...
# read in csv file
    ...
    filtered_data = [d for d in data if any([True if (c.lower() in d[0].lower()) else False for c in desired_column_names])]

    #loop through each sublist with a comprehension

    filtered_data1= [[x.strip() for x in d if x.strip() != ''] for d in filtered_data]
    ...
    if not os.path.exists(args.output):
        ...
        for i, row in enumerate(header_index_dict):
            for col in clean_data_dict:
                output_header_row.append((col+'_'+row).replace(' ','_').replace(':',''))
